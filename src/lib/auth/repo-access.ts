...
// integrations is the most-restricted repo, included only on Scale.
//
// See recon/src/lib/auth/repo-access.ts for the full design rationale.

import type { CurrentTenant } from "./session";

const THIS_REPO_NAME = "integrations";

// Only Scale plan includes integrations. Plaid + Stripe + Gusto +
// Bill.com connectors are the premium tier.
...
export class RepoNotIncludedError extends Error {
  constructor(
    ...
    super(
      `The ${repoName} module is not included in your "${currentPlan}" plan. Upgrade to Scale at /admin/billing.`
    );
