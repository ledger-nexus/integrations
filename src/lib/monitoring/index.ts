...
// (PR #28). FINAL companion-repo port — closes the integrations
// half of portfolio-wide deficiency #5 (No Sentry / no error
// tracking; Medium severity). After this PR + the other 3 land,
// deficiency #5 flips Remediated → Closed portfolio-wide.
//
// Why this exists:
...
//      stack — which on integrations regularly embeds Plaid
//      transaction data, OAuth tokens, account numbers. Going through
//      this shim runs `redactPii()` first.
//      This is especially load-bearing here: a leaked OAuth token in
//      Sentry's UI is a Critical incident.
 ...
 * objects, emails, tokens, or transaction descriptions — the shim
 * redacts field-name-matched keys but a free-text `message: "Plaid
 * sync failed for token plk-abc"` slips through. Keep context
 * structured.
 */
export interface ErrorContext {
  /** Coarse-grained category for triage ("connector", "webhook", "sync"). */
  context?: string;
...
export function captureError(err: unknown, context: ErrorContext = {}): void {
  ...
  //
  // We deliberately do NOT pass `err` directly — Plaid errors echo
  // request/response payloads in `.message`, and Prisma errors echo
  // column values. console.error would print them verbatim. Instead
  // extract err.name + err.code and let redactPii handle the rest.
  const summary =
...
/**
 * Capture a non-error event (e.g., "webhook received for already-
 * removed item"). Use for security-relevant observations that aren't
